...
<Aside>
	Adding EmDash to an existing Astro project? Follow the [dedicated
	guide](/existing-project/) — it covers the peer dependencies, required
	integrations, and deployment caveats step by step.
</Aside>
...
EmDash sites run in SSR mode by default, so content is served at runtime and changes appear immediately.

For intentionally static pages with `getStaticPaths`, content is fetched at build time and stays frozen until the next rebuild:
...
For dynamic CMS pages, set `prerender = false` to fetch content on each request:
...
<Aside type="tip">
	Use server rendering for starter themes, reusable templates, and frequently updated content. Use
	static generation only for content that changes rarely and benefits from rebuild-time output.
</Aside>

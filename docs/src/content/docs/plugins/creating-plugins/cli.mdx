...
## Delegated release setup

Sign in locally, then prepare the package profile and GitHub Actions workflow with the following commands:
...
If the package profile is missing, `release setup` offers to create it from `emdash-plugin.jsonc`. For an existing profile without delegated-release settings, it adds the signed GitHub repository and release policy while preserving the package metadata. The default policy asks the signed-in Atmosphere account to approve releases when plugin permissions increase. You can choose to require approval for every release instead.

Set `repo` in `emdash-plugin.jsonc`, or enter the canonical GitHub repository URL when prompted. Run `emdash-plugin profile setup` when you need to prepare only the profile.

In a non-interactive environment, profile changes require `--yes`. A GitHub Actions release fails with `PACKAGE_PROFILE_REQUIRED` before uploading files when its signed profile is missing or links a different repository. Run profile setup locally, then start the workflow again.

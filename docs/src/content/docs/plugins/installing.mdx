...
1. **Sandbox runner configured** — Marketplace plugins run in an isolated runtime, which requires the sandbox runner. The following configuration enables it:
   ...
   On **Cloudflare Workers**, sandboxing uses the Dynamic Worker Loader API and requires a D1 database binding for sandbox bridge operations. On **Node.js**, install the workerd sandbox runner:

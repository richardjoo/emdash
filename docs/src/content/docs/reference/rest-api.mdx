...
- an authenticated admin session for same-origin requests
...
State-changing requests (`POST`, `PUT`, `PATCH`, `DELETE`) that rely on the session must
also include `X-EmDash-Request: 1` for CSRF protection. Bearer-token requests are exempt. The
admin UI and `EmDashClient` add this header automatically.
...
`_rev` is optional. Pass a token returned by a get, create, or update response to reject stale
writes with `409 Conflict`; omit it for a blind write. Create and update requests accept only
`status: "draft"`. To publish an entry, send

...
- a session cookie (`emdash_session`) from the admin UI or another same-site client
- a Bearer token in the `Authorization` header

Bearer-token requests look like this:

```http
Authorization: Bearer <token>
```

State-changing requests (`POST`, `PUT`, `PATCH`, `DELETE`) that rely on the session cookie must
also include `X-EmDash-Request: 1` for CSRF protection. Bearer-token requests are exempt. The
admin UI and official EmDash clients add this header automatically.
...
Publishing is a separate action. Use the publish endpoint to move a draft live.
...
| ----- | ---- | ----------- |
| `file` | file | Required upload payload |
| `fieldId` | string | Optional field-specific MIME allowlist selector |
| `width` / `height` | number | Optional client-known image dimensions |
| `thumbnail` | file | Optional small preview image for placeholder generation |
...
```json
{
	"data": {
		"item": {
			...
			"width": 1920,
			"height": 1080,
			"url": "/_emdash/api/media/file/01HXK5MZSN.jpg"
		}
	}
}
```
...
This returns a pending media record plus the signed upload details:
...
Rebuilds the FTS index for a single collection.

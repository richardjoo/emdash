...
EmDash content works with both static and server-rendered pages. Use static generation only when you intentionally want content frozen at build time. Use server rendering for starter themes, reusable templates, and sites where admin edits should appear immediately.

### Static (Prerendered)

For static pages, use `getStaticPaths` to generate routes at build time. New or edited CMS entries do not appear until the site rebuilds:
...
For server-rendered pages, query content directly on each request:

...
<Aside type="caution">
	Deleting content moves it to the trash first. You can restore it later, or permanently delete it
	from the trash when you no longer need it.
</Aside>
...
The following request updates an existing post:
...
The following request soft-deletes a post and moves it to the trash:
...
```http
POST /_emdash/api/content/posts/my-new-post/restore
Authorization: Bearer YOUR_API_TOKEN
```
...
```http
DELETE /_emdash/api/content/posts/my-new-post/permanent
Authorization: Bearer YOUR_API_TOKEN
```

...
<Aside>
	**The Cloudflare sandbox runner is D1-only.** Its plugin bridge talks to a D1 binding directly,
	independent of the configured adapter, so marketplace/sandboxed plugins are not available on a
	Hyperdrive-backed Cloudflare deployment. Node.js deployments can use the workerd sandbox runner
	against their configured database.
</Aside>
